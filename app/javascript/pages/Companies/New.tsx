...
interface CompaniesNewProps {
  errors?: CompanyErrors;
  company?: { name?: string; cnpj?: string };
}

const CompaniesNew: React.FC<CompaniesNewProps> = ({
  errors: initialErrors,
  company: initialCompany,
}) => {
  const initialCNPJ = initialCompany?.cnpj || "";
  ...
  // Update mask when initial company changes
  useEffect(() => {
    if (initialCompany?.cnpj !== undefined) {
      cnpjMask.setValue(initialCompany.cnpj);
    }
  }, [initialCompany?.cnpj, cnpjMask.setValue]);
  ...
  return (
    <Layout>
      ...
      <div className="px-4 py-6 sm:px-0">
        <h1 className="text-3xl font-bold text-gray-900 mb-6">Nova Empresa</h1>
